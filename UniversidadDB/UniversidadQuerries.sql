...
SELECT CONCAT (p.nombre," ",apellido1," ", apellido2) AS profesor, d.nombre AS departamento, a.nombre AS asignatura FROM persona p JOIN profesor pr ON  p.id = pr.id_profesor JOIN departamento d ON pr.id_departamento = d.id LEFT JOIN asignatura a ON a.id_profesor = pr.id_profesor WHERE a.id_profesor IS NULL;
